...
export const columnResolvers = {
  Mutation: {
    createColumn: async (_: any, { boardId, title, order }: any, ctx: GraphQLContext) => {
    ...
    updateColumn: async (_: any, { id, title, order }: any, ctx: GraphQLContext) => {
      if (!ctx.userId) throw new Error('Not authenticated')
    ...
    deleteColumn: async (_: any, { id }: any, ctx: GraphQLContext) => {
      if (!ctx.userId) throw new Error('Not authenticated')
